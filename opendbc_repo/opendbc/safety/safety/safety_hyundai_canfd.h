...
#define HYUNDAI_CANFD_COMMON_RX_CHECKS(pt_bus)                                                                      \
  {.msg = {{0x35, (pt_bus), 32, .ignore_checksum = true, .max_counter = 0xffU, .frequency = 100U},                  \
  ...
  {.msg = {{0xea, (pt_bus), 24, .ignore_checksum = true, .max_counter = 0xffU, .frequency = 100U}, { 0 }, { 0 }}},  \
...
static bool hyundai_canfd_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    ...
    bool is_angle_msg = ((addr == 0xcb) && hyundai_camera_scc && !hyundai_canfd_lka_steering);

    block_msg = is_lka_msg || is_lfa_msg || is_lfahda_msg || is_scc_msg || is_angle_msg;
  }
