...
static const CanMsg HYUNDAI_COMMUNITY_TX_MSGS[] = {
  {0x340, 0, 8, false},  // LKAS11 Bus 0
  {0x4F1, 0, 4, false}, // CLU11 Bus 0
  {0x485, 0, 4, false}, // LFAHDA_MFC Bus 0
  {0x4F1, 2, 4, false}, // CLU11 Bus 2
  {0x251, 2, 8, false},  // MDPS12 Bus 2
  ...
  {0x4A2, 0, 8, false}, // FRT_RADAR11 Bus 0
};

#define HYUNDAI_COMMUNITY_RX_CHECKS(legacy)                                                                                                            \
...
bool hyundai_community_legacy = false;
...
static bool hyundai_community_tx_hook(const CANPacket_t *to_send) {
  ...
  if (addr == 0x340) {
    ...
    const TorqueSteeringLimits limits = hyundai_alt_limits ? HYUNDAI_COMMUNITY_STEERING_LIMITS_ALT : HYUNDAI_COMMUNITY_STEERING_LIMITS;
...
static safety_config hyundai_community_init(uint16_t param) {
  ...
  hyundai_community_legacy = false;
  ...
  if (hyundai_longitudinal) {
    static RxCheck hyundai_community_long_rx_checks[] = {
      HYUNDAI_COMMUNITY_RX_CHECKS(false)
...
static safety_config hyundai_community_legacy_init(uint16_t param) {
  //const int HYUNDAI_PARAM_CANFD_LFA_ENG = 64;

  // older hyundai models have less checks due to missing counters and checksums
  static RxCheck hyundai_community_legacy_rx_checks[] = {
    {.msg = {{0x260, 0, 8, .max_counter = 3U, .frequency = 100U},
            {0x371, 0, 8, .ignore_checksum = true, .ignore_counter = true, .frequency = 100U}, { 0 }}},
  ...
  hyundai_common_init(param);
  hyundai_community_legacy = true;
  hyundai_longitudinal = false;
  hyundai_camera_scc = false;
  //hyundai_controls_allowed = GET_FLAG(param, HYUNDAI_PARAM_CANFD_LFA_ENG);
  hyundai_controls_allowed = true;
  return BUILD_SAFETY_CFG(hyundai_community_legacy_rx_checks, HYUNDAI_COMMUNITY_TX_MSGS);
}
...
const safety_hooks hyundai_community_legacy_hooks = {
  .init = hyundai_community_legacy_init,
  .rx = hyundai_community_rx_hook,
  .tx = hyundai_community_tx_hook,
  .fwd = hyundai_community_fwd_hook,
};
